...
public static class ResourceHelpers
{
    ...
    public static void ConsumeResources(List<ResourceRequirement> toConsume)
    {
        if (DebugCheats.Instance.CareAboutResourcesNeeded() == false)
        {
            return;
        ...
        for(int x=0;x<toConsume.Count;x++)
        {

            ResourceManager.Instance.ReduceQuantity(toConsume[x].ResourceName,toConsume[x].QuantityRequired);
        }
    }
